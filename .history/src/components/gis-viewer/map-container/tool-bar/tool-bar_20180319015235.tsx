...
import { TOOL_BAR_TAG, DRAW_BAR_PLUGIN_TAG, ZOOM_TO_EXTENT_PLUGIN_TAG, SEARCH_PLUGIN_TAG, MEASURE_PLUGIN_TAG, LAYERS_MANAGER_PLUGIN_TAG, FULL_SCREEN_PLUGIN_TAG } from '../../../../utils/statics';
...
export class ToolBar {
    ...
    private toolbarFeaturesDecision() {
        ...
        //     [LAYERS_MANAGER_PLUGIN_TAG]: [controllerMapGroup],
        ...
        _.forEach(toolbarPlugins.children, (plugin: HTMLElement) => {
            ...
            if (plugin.tagName.toLowerCase() === LAYERS_MANAGER_PLUGIN_TAG) {
                debugger
            }

            if (plugin.tagName.toLowerCase() === DRAW_BAR_PLUGIN_TAG) {
                // let c: HTMLElement = (plugin as HTMLDrawBarPluginElement).control.getContainer();
                let cc: HTMLElement = (plugin as HTMLDrawBarPluginElement).getControl().getContainer();
                console.log(cc);
            ...
            if (plugin.tagName.toLowerCase() === ZOOM_TO_EXTENT_PLUGIN_TAG) {
                // debugger
                const container: HTMLElement = (plugin as HTMLZoomToExtentPluginElement).getControl().getContainer();
                Utils.stopDoubleClickOnPlugin((plugin as HTMLZoomToExtentPluginElement).getControl().getContainer());
            }

            if (plugin.tagName.toLowerCase() === FULL_SCREEN_PLUGIN_TAG) {
                debugger
            }

            if (plugin.tagName.toLowerCase() === MEASURE_PLUGIN_TAG) {
                debugger
                const measure: HTMLElement = (plugin as HTMLMeasurePluginElement).getControl().getContainer();
                measure.classList.add('polyline-measure');
            }

            if (plugin.tagName.toLowerCase() === SEARCH_PLUGIN_TAG) {
                debugger
            }
