...
export class MeasurePlugin {
    ...
    private pluginUnitParser(globalUnit: DistanceUnitType): string {
    ...
    private pluginUnitOnButtonParser(globalUnit: DistanceUnitType): string {
        // 'metres', 'landmiles', 'nauticalmiles'
        switch (globalUnit.toLowerCase()) {
            case 'km':
                return 'm';
    ...
    private setUnits(units: DistanceUnitType) {
        const unitControlIdElement = document.getElementById('unitControlId');
        if (!unitControlIdElement) return;
        
        let pluginUnitOptions: string = this.pluginUnitOnButtonParser(units);

        while (unitControlIdElement.innerText === )
